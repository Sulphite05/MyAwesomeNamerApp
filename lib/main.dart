  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton( 
            onSelected: (value) async{  // value is a an enumeration value, whatever I select from the enums values
    ...
    builder: (context){
      return AlertDialog(
        ...
        actions: [
          ...
          TextButton(onPressed: (){
            Navigator.of(context).pop(true);  // to return a value to the show dialogue
          }, 
          child: const Text('Sign Out')),
        ],
      );
    }
    ).then((value) => value ?? false); // show dialogue's value can be null, for instance, i press somewhere else on the screen instead of the pop up
}
